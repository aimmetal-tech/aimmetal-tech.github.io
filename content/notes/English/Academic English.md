---
title: 学术英语
tags:
  - 英语
  - 学术
---

## Week1

### Scientific Reseach五个特征

- Observable 可观察
- Tentative 暂时性
- Testable 可检验
- Replicable 可重复
- Social 社会性

### Types of academic publications

- **Journal**期刊
- **Transactions**会刊/会报
- **Conference**会议论文集

### 学术不端 Academic Misconduct

- plagiarism 抄袭。
- self-plagiarism 自我抄袭，包括salami slicing/salami publication，把原本属于一项完整研究的数据或成果，刻意拆分成好几篇零碎的微型论文来发表，以此刷论文篇数。
- misrepresentation 失实陈述
- fabrication 编造数据或结果
- falsification 篡改

### Research Paper vs. Review Paper

| 项目       | Research paper     | Review paper         |
| ---------- | ------------------ | -------------------- |
| 内容       | 原创研究/实验结果  | 总结、评价已有研究   |
| 标题/摘要  | 常体现 experiments | 常有 review / survey |
| headings   | 通常有标准结构     | 不一定有标准结构     |
| 图表       | 原始数据相关       | 多来自引用文献       |
| references | 较少，30–40        | 很多，150–300        |

## Week2

### 3-Pass Reading Strategy

1. 1-pass 快速了解论文大意，判断是否值得继续读。
   title abstract intro headings
   mathematical content briefly
   conclusion
   references
   1-pass 找5c：category context correctness contribution clarity
2. 2-pass 理解论文主线和支撑证据。
3. 3-pass 深入理解，几乎重新实现论文。

## Week2-2

### Literature Review

FOCUS

- F = Focused and Selective
- O = Organized and Thematically
- C = Critical and Evaluative
- U = Unified and Coherent
- S = Synthesized not Summarized

## Week3

### Title

Keywords prominently

### Abstract摘要结构：BPMRC

- B Background
- P Purpose
- M Methodology
- R Results
- C Conclusion

## Week4

### 1. Introduction 引言 Six Stages

背景、研究领域重要性 -> 前人研究 -> 问题/不足/Gap -> 本文的目的或方法 -> 本文贡献 -> 文章结构安排

### 2. Methods

info in method section

1. purpose
2. data collection
3. data analysis
4. selection criteria
5. reproducibility
6. understanding results
7. validity/credibility

### 3. Results 结果部分：LHC

- L: Locate
- H: Highlight
- C: Comment
